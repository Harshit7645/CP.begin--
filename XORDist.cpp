...
long long solve(int N,vector<vector<int>>A,int k)
{
    vector<int>x(N),y(N);
    rep(i,0,N)
    {
    ...
    rep(val,0,101)
    {
        rep(i,0,N)
        {
            if(m1[(x[i]^val)] && m2[(y[i]^(k-val))] && m1[(x[i]^val)]==m2[(y[i]^(k-val))])
            {
                cnt++;
                cout<<x[i]<<" "<<y[i]<<" "<<(x[i]^val)<<" "<<(y[i]^val);
            }
            m1[x[i]]++;
            m2[y[i]]++;
        }
...
int main()
{
    cout<<solve(4,{{0,0},{0,1},{0,0},{1,0}},0);
}   