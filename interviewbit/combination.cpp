...
void backtrack(vector<int>&A,int ind,vector<vector<int>>&ans,vector<int>&row,int k)
{
    if(row.size()==k)
    ans.push_back(row);
    rep(i,ind,A.size())
...
vector<vector<int>>solve(int n,int k)
{
    ...
    backtrack(A,0,ans,row,k);
    return ans;
}
int main()
{
    vector<vector<int>>ans=solve(4,2);
    rep(i,0,ans.size())
    {
        rep(j,0,ans[i].size())
        {
