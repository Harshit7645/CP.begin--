...
bool good(vector<pair<ll,ll>>&v,ll k)
{
    ll currlow=0,currhigh=0,valid=1;
    rep(i,0,v.size())
    {
        currlow=max(currlow,v[i].first);
        currhigh=min(currhigh+k,v[i].second);
        if(currlow>currhigh)
        {
            valid=0;
            break;
        }
        currlow=max(currlow-k,0ll);
    }
    return valid;
}
int main()
