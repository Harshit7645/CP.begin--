...
int main()
{
    ...
    while(tt--)
    {
        ...
        while (!pq.empty())
        {
            ...
            pq.pop();
            auto [curr,prev]=p;
            //ll node=tp.second.first,prev=tp.second.second,dist=tp.first;
            if(dp[curr][prev]<dist)
            continue;
            if(s[curr]<s[prev])
            prev=curr;
            for(auto [to,w]:v[curr]) 
            {   
                auto nextdist=dist+w*s[prev];
                if(dp[to][prev]>nextdist)
                {
                    dp[to][prev]=nextdist;
                    pq.push({nextdist,{to,prev}});
                }
            }  
        }
        ll ans=1e18;
