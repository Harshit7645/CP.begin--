...
int main()
{
    ...
    rep(i,0,q)
    {
        ...
        if(low>arr[0])
        {
            it1=lower_bound(arr.begin(),arr.end(),low);
            f=1;
        }
        else
        x=arr[0];
        if(high<arr[n-1])
        {
            it2=upper_bound(arr.begin(),arr.end(),high);
            f1=1;
        }
        else
        {
        ...
        //cout<<*it1<<" "<<*it2<<"\n";
        //cout<<x<<" "<<y<<"\n";
        sleep=postsleep[m[x]]-postsleep[m[y]];
        if(m[x]%2==0)
        {
            sleep+=(x-low);
        }
